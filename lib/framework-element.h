...
static js_value_t *
bare_win_ui_framework_element_vertical_alignment(js_env_t *env, js_callback_info_t *info) {
  int err;
  ...
  try {
    if (argc == 1) {
      err = js_create_int32(env, int32_t(element.VerticalAlignment()), &result);
      assert(err == 0);
    } else {
      int32_t alignment;
      if (!bare_win_ui__read_int32(env, argv[1], "alignment", &alignment)) return nullptr;

      element.VerticalAlignment(VerticalAlignment(alignment));
    }
  } catch (hresult_error const &error) {
    bare_win_ui__throw(env, error);
...
static js_value_t *
bare_win_ui_framework_element_horizontal_alignment(js_env_t *env, js_callback_info_t *info) {
  int err;
  ...
  try {
    if (argc == 1) {
      err = js_create_int32(env, int32_t(element.HorizontalAlignment()), &result);
      assert(err == 0);
    } else {
      int32_t alignment;
      if (!bare_win_ui__read_int32(env, argv[1], "alignment", &alignment)) return nullptr;

      element.HorizontalAlignment(HorizontalAlignment(alignment));
    }
  } catch (hresult_error const &error) {
    bare_win_ui__throw(env, error);
