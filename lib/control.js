...
module.exports = exports = class WinUIControl extends FrameworkElement {
  ...
  set padding(value) {
    if (typeof value === 'number') binding.controlPadding(this._tag, value)
    else {
      const { left = 0, top = 0, right = 0, bottom = 0 } = value

      binding.controlPadding(this._tag, left, top, right, bottom)
    }
  }

  get horizontalContentAlignment() {
    return binding.controlHorizontalContentAlignment(this._tag)
  }

  set horizontalContentAlignment(value) {
    binding.controlHorizontalContentAlignment(this._tag, value)
  }

  get verticalContentAlignment() {
    return binding.controlVerticalContentAlignment(this._tag)
  }

  set verticalContentAlignment(value) {
    binding.controlVerticalContentAlignment(this._tag, value)
  }
